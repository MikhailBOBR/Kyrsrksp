...
| --- | --- | --- | ---: |
| `api-crud-coverage.test.js` | covers goals read and manual update flows | passed | 169.80 |
| `api-crud-coverage.test.js` | covers product search, update, delete and not-found branches | passed | 102.62 |
| `api-crud-coverage.test.js` | covers meals, templates, recipes and planner destructive flows | passed | 146.57 |
| `api-crud-coverage.test.js` | covers daily controls, wellbeing, metrics and note deletion | passed | 123.14 |
| `api-crud-coverage.test.js` | covers shopping and favorites create, filter, clear and remove flows | passed | 146.49 |
| `api.test.js` | registers a new user and returns token | passed | 147.92 |
| `api.test.js` | denies dashboard access without auth token | passed | 8.22 |
| `api.test.js` | allows demo user to access extended dashboard | passed | 98.36 |
| `api.test.js` | returns goal presets and applies selected preset | passed | 94.40 |
| `api.test.js` | prevents regular user from creating products | passed | 88.42 |
| `api.test.js` | allows admin to create product | passed | 82.71 |
| `api.test.js` | prevents regular user from updating and deleting admin catalog entries | passed | 184.28 |
| `api.test.js` | creates meal for user and returns it in filtered list | passed | 85.34 |
| `api.test.js` | tracks hydration and returns updated summary | passed | 86.09 |
| `api.test.js` | creates and applies meal template | passed | 91.65 |
| `api.test.js` | creates recipe, applies it and sends it to planner | passed | 89.70 |
| `api.test.js` | creates day note and returns it through dashboard | passed | 93.20 |
| `api.test.js` | creates wellbeing checkin and returns readiness summary | passed | 95.11 |
| `api.test.js` | creates body metric entry and exposes summary | passed | 85.22 |
| `api.test.js` | creates meal plan from template and marks it completed | passed | 92.42 |
| `api.test.js` | generates weekly plan from templates and recipes | passed | 85.60 |
| `api.test.js` | adds product to shopping list and toggles checked state | passed | 99.31 |
| `api.test.js` | adds favorites for product and template | passed | 98.70 |
| `api.test.js` | rejects unsupported daily report export format | passed | 78.73 |
| `api.test.js` | exposes extended OpenAPI document | passed | 8.04 |
| `auth-session.test.js` | registers a user and exposes profile through /api/auth/me | passed | 194.34 |
| `auth-session.test.js` | rejects duplicate registration for the same email | passed | 102.39 |
| `auth-session.test.js` | returns admin role in the current session profile | passed | 86.16 |
| `client-actions.test.js` | keeps drawer navigation views in markup | passed | 2.02 |
| `client-actions.test.js` | wires static toolbar, drawer and import actions | passed | 0.59 |
| `client-actions.test.js` | keeps dynamic card actions connected for key tabs | passed | 1.01 |
| `client-actions.test.js` | keeps hidden drawer layers non-interactive | passed | 0.38 |
| `client-static.test.js` | keeps critical app shell controls available | passed | 2.16 |
| `client-static.test.js` | keeps navigation and theme style rules in place | passed | 0.37 |
| `client-static.test.js` | keeps client javascript valid and export flow wired | passed | 3.45 |
| `client-static.test.js` | passes the shared client contract suite | passed | 3.02 |
| `contracts.test.js` | rejects invalid login and invalid registration payloads | passed | 162.21 |
| `contracts.test.js` | rejects invalid meal payloads | passed | 86.66 |
| `contracts.test.js` | rejects invalid recipe and planner payloads | passed | 92.76 |
| `contracts.test.js` | rejects invalid goal updates and invalid authorization token | passed | 89.17 |
| `exports.test.js` | returns json daily report with goals, meals and hydration summary | passed | 177.67 |
| `exports.test.js` | rejects unsupported daily report formats | passed | 99.32 |
| `exports.test.js` | rejects export requests with invalid dates | passed | 91.39 |
| `fuzz.test.js` | fuzzes auth registration with random invalid payloads without 500 errors | passed | 318.31 |
| `fuzz.test.js` | fuzzes malformed json bodies across mutating endpoints without 500 errors | passed | 203.91 |
| `fuzz.test.js` | fuzzes protected business endpoints without server crashes | passed | 662.38 |
| `fuzz.test.js` | fuzzes daily, import and shopping endpoints without server crashes | passed | 1162.69 |
| `imports.test.js` | previews and applies meal import from tsv | passed | 171.25 |
| `imports.test.js` | downloads import template in tsv format | passed | 88.53 |
| `imports.test.js` | prevents regular user from importing products | passed | 81.96 |
| `imports.test.js` | allows admin to import products from json | passed | 86.21 |
| `observability.test.js` | health endpoint returns release metadata and request id header | passed | 48.77 |
| `observability.test.js` | liveness and readiness endpoints separate process and backing-service checks | passed | 10.96 |
| `observability.test.js` | echoes incoming request id and returns it on 404 | passed | 7.68 |
| `observability.test.js` | rejects new requests while runtime is draining | passed | 14.50 |
| `openapi-docs.test.js` | keeps the branded Swagger document readable and complete | passed | 2.53 |
| `openapi-docs.test.js` | documents every operation with tags, operationId, success and error responses | passed | 1.58 |
| `openapi-docs.test.js` | keeps path parameters and protected routes explicit | passed | 1.57 |
| `openapi-docs.test.js` | keeps Swagger UI styled for the project and useful for manual checks | passed | 0.18 |
| `openapi-docs.test.js` | keeps README, API docs and wiki linked to the current project surface | passed | 0.75 |
| `runtime-config.test.js` | config prefers environment variables over yaml file values | passed | 6.78 |
| `runtime-config.test.js` | config accepts standard platform host and port aliases | passed | 1.11 |
| `runtime-config.test.js` | config exposes runtime and backing service tuning from the environment | passed | 0.96 |
| `runtime-config.test.js` | production config rejects default JWT secrets | passed | 1.23 |
| `runtime-config.test.js` | structured logger serializes error payloads | passed | 1.50 |
| `runtime-config.test.js` | postgres query adapter preserves mixed-case response aliases | passed | 0.25 |
| `security.test.js` | hashes passwords and validates correct credentials | passed | 226.71 |
| `security.test.js` | signs and verifies access tokens with role payload | passed | 8.10 |
| `security.test.js` | rejects tampered access tokens | passed | 3.46 |
| `smoke.test.js` | serves health endpoint | passed | 102.57 |
| `smoke.test.js` | serves openapi and swagger ui | passed | 39.32 |
| `smoke.test.js` | serves client html, styles and favicon | passed | 33.27 |
| `smoke.test.js` | returns json 404 for unknown api route | passed | 10.32 |
| `surface-coverage.test.js` | covers every API operation, server module, frontend contract and fuzz scenario | passed | 8.20 |
| `validation.test.js` | accept valid dates and reject impossible calendar values | passed | 3.90 |
| `validation.test.js` | accept valid times and reject impossible hour-minute combinations | passed | 0.81 |
| `validation.test.js` | validates email and password rules | passed | 0.77 |
| `validation.test.js` | validates numeric, meal type and category constraints | passed | 2.30 |
| `validation.test.js` | shifts dates forward and backward | passed | 2.29 |
| `validation.test.js` | builds ordered date windows | passed | 2.64 |
