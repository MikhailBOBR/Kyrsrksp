...
- отчет 100% функционального покрытия в [11-test-coverage-report.md](./11-test-coverage-report.md).

### 5. Фаззинг-тестирование

- фаззинг вынесен в отдельный сценарий [fuzz.test.js](../server/tests/fuzz.test.js);
- проверяются некорректные payload-ы регистрации, защищённых бизнес-маршрутов, импорта, ежедневных сущностей, покупок и админских операций;
- отдельный adversarial-сценарий покрывает 100 API request shapes: query-параметры, body-shape, path id, authorization и content-type края;
